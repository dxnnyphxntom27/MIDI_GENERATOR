...
# 🎼 Losowa sekwencja startowa z prawdziwej piosenki
...
# 🚫 Usuń powtarzające się instrumenty (opcjonalnie)
seen_instruments = set()
filtered_tokens = []
for token in tokens:
    if token.startswith("instrument_"):
        if token in seen_instruments:
            continue
        seen_instruments.add(token)
    filtered_tokens.append(token)

# 💾 Zapis tokenów (jeden wiersz, jak dane treningowe)
with open(OUTPUT_PATH, "w") as f:
    f.write(" ".join(filtered_tokens))

print(f"✅ Wygenerowano {len(filtered_tokens)} tokenów → zapisano do {OUTPUT_PATH}")
