...
# === PARAMETRY GENERACJI ===
MAX_TOKENS = 100
START_TOKENS = ["key_unknown", "tempo_140", "track_0", "instrument_0"]
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
TIME_RESOLUTION = 24  # ticks per beat
TEMPERATURE = 1     # 0 -> argmax, >0 -> sampling
TOP_K = 50
STOP_TOKEN = "<SONG_END>"
...
def tokens_to_midi(tokens, chord_id_to_notes, output_path,
    ...
    while i < N:
        token = tokens[i]
        if token is None:
            print(f"DEBUG: token[{i}] = None, pomijam.")
            i += 1
            continue

        # ---- DEBUG: Tempo ----
        if isinstance(token, str) and token.startswith("tempo_"):
            print(f"\n--- DEBUG TEMPO: Przetwarzam token[{i}] = '{token}' ---")
            try:
                bpm = float(token.split("_", 1)[1])
                print(f"Parsed BPM = {bpm}")
            except Exception as e:
                print(f"Błąd parsowania tempo z '{token}': {e}")
                bpm = current_tempo
            ...
            if bpm < min_tempo or bpm > max_tempo:
                ...
                clamped = True
            if clamped:
                print(f"ZACLAMPowane tempo: {bpm}")
            else:
                print(f"Tempo nie wymaga clampowania: {bpm}")

            current_tempo = bpm
            print(f"Ustawiam tempo {current_tempo} BPM na ticku {current_tick}")
            # Dodaj tempo change do struktury pretty_midi
            pm._tick_scales.append(
                (current_tick, 60.0 / current_tempo / time_resolution)
            )
            i += 1
            continue
        ...
        if isinstance(token, str) and token.startswith("track_"):
            try:
                ...
                print(f"Błąd parsowania track z '{token}': {e}")
                current_track_idx = None
            print(f"DEBUG: Wybrano ścieżkę track_{current_track_idx}")
            if current_track_idx is not None and current_track_idx not in instruments:
                ...
                print(f"DEBUG: Utworzono instrument dla track_{current_track_idx}")
            i += 1
            continue
        ...
        if isinstance(token, str) and token.startswith("instrument_") and current_track_idx is not None:
            try:
                ...
                print(f"DEBUG: Ustawiam instrument program={program_num} dla track_{current_track_idx}")
            except Exception as e:
                print(f"Błąd parsowania instrument z '{token}': {e}")
            i += 1
            continue
        ...
        if isinstance(token, str) and token.startswith("time_shift_"):
            try:
                ...
                print(f" --- DEBUG: time_shift = {shift} ticks")
            except Exception as e:
                print(f"Błąd parsowania time_shift z '{token}': {e}")
                shift = 0
            print(f"DEBUG: time_shift_{shift} | Przesuwam current_tick {current_tick} -> {current_tick + shift}")
            current_tick += shift
            cnt_time_shift += 1
            i += 1
            continue
        ...
        if isinstance(token, str) and token.startswith("duration_") and current_track_idx is not None:
            try:
                ...
                print(f"Błąd parsowania duration z '{token}': {e}")
                dur = time_resolution
            ...
            print(f"DEBUG: duration dla track_{current_track_idx}: {dur} ticks")
            i += 1
            continue
        ...
        if isinstance(token, str) and token.startswith("chord_") and current_track_idx is not None:
            try:
                ...
                print(f"Błąd parsowania chord z '{token}': {e}")
                chord_id = None
            if chord_id is not None and chord_id in chord_id_to_notes:
                ...
                print(f"DEBUG: chord_{chord_id} (notes={notes}) @tick={current_tick} (start={start_seconds:.3f}s, end={end_seconds:.3f}s, tempo={current_tempo})")
                for pitch in notes:
                    p = int(pitch)
                    if 0 <= p <= 127:
                ...
                print(f"DEBUG: chord_{chord_id} nie znaleziono w chord_id_to_notes")
            next_token = tokens[i+1] if (i+1) < N else None
            if not (isinstance(next_token, str) and next_token.startswith("time_shift_")):
                dur_ticks = durations.get(current_track_idx, time_resolution)
                print(f"DEBUG: chord_{chord_id} przesuwa current_tick o {dur_ticks} (bo brak time_shift po nim)")
                current_tick += dur_ticks
            i += 1
            continue

        if token == "<SONG_END>":
            print(f"DEBUG: <SONG_END> @tick={current_tick} -> przesuwam tick o 1 beat ({time_resolution})")
            current_tick += time_resolution
            i += 1
            continue

        print(f"DEBUG: pomijam nieznany token[{i}]: '{token}'")
        i += 1
