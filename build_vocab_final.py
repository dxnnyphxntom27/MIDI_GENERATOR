...
# Obsługiwane typy tokenów
ALLOWED_PREFIXES = ("chord_", "time_shift_")
...
def build_token_vocab(token_dir):
    ...
    for artist_dir in artist_dirs:
        print(f"🎼 Przetwarzam: {artist_dir.name}")
        ...
        print(f" - Skończono {artist_dir.name} — unikalnych tokenów: {len(vocab)}\n")

    sorted_vocab = sorted(list(vocab))
    token_to_id = {token: idx for idx, token in enumerate(sorted_vocab)}
    id_to_token = {idx: token for token, idx in token_to_id.items()}

    print(f"🏁 Gotowe! Przetworzono {total_files} plików. Tokenów: {len(token_to_id)}")
